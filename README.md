...
Full documentation — including guides for adding and harmonising
datasets — is at
<https://motivation-and-behaviour.github.io/bigger_picture/>. The
scientific background and preregistration are in `docs/prereg/`.
...
`harmonisation/datasets/BPIPD-<id>/dataset.yaml` describing its files
(`resources`, or `waves` for multi-wave studies), their roles (`data`,
`codebook`, `docs`), and how to read them (`reader`, plus
reader-specific keys such as `sheet` or `table`). The intended contract
is documented by `harmonisation/dataset.schema.json`; the pipeline reads
YAML specs directly and does not yet validate them against the JSON
schema at runtime.

See the documentation site for the [field-by-field
reference](https://motivation-and-behaviour.github.io/bigger_picture/guides/dataset-yaml.html)
and a [cookbook of spec
patterns](https://motivation-and-behaviour.github.io/bigger_picture/guides/spec-cookbook.html)
for common dataset layouts.

## Harmonisation Contract

Harmonisation is table-driven: the shared schema lives in
`harmonisation/dataschema.csv`, each dataset maps onto it via
`harmonisation/datasets/BPIPD-<id>/variables.csv`, and recode tables
live in shared (`harmonisation/lookups/`) or dataset-specific
(`BPIPD-<id>/lookups/`) CSVs. Mapping expressions are evaluated against
the tidied study tibble in a sandboxed environment: bare function calls
resolve from base R plus the curated allowlist in
`bp_harmonisation_functions()`
(`R/harmonise/harmonisation_functions.R`), and namespaced calls such as
`haven::as_factor()` are also allowed. Per-dataset harmonised tibbles
are row-bound into `analysis_data`.

The [harmonisation
guide](https://motivation-and-behaviour.github.io/bigger_picture/guides/harmonisation.html)
covers the full workflow (statuses, expressions, lookups, validation),
and the [dataschema
reference](https://motivation-and-behaviour.github.io/bigger_picture/reference/dataschema.html)
is generated from `dataschema.csv` on every site build.
...
4.  Create `R/tidy/tidy_BPIPD_<id>.R`. The pipeline requires one tidier
    file per study and tracks that file as a target input.
5.  Run `targets::tar_make()`.

The [adding a dataset
guide](https://motivation-and-behaviour.github.io/bigger_picture/guides/adding-a-dataset.html)
walks through this workflow in full, including branching, validation,
...
- This repo uses `README.Rmd` as the source of truth; `README.md` is
  generated.
- The documentation site source lives in `docs/` (Quarto). Preview
  locally with `quarto render docs`; CI builds it on every PR and
  deploys to GitHub Pages on merge to `main`. Generated pages
  (dataschema, progress, the reader and allowlist tables) read
  repository files at build time, so they never need manual updates.
...
- Documentation site:
  <https://motivation-and-behaviour.github.io/bigger_picture/> (source
  in `docs/`, built by `.github/workflows/docs.yaml`, deployed to GitHub
  Pages)
